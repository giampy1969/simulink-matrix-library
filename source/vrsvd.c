/*  S-Function for Simulink Real Matrix Singular Value Decomposition **************************/
/*  Giampiero Campa August-00 *****************************************************************/

#define S_FUNCTION_NAME vrsvd
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    /* first, second and third output ports width                                             */
	ssSetOutputPortWidth(S,0,(int_T)(n[0]*(MIN(n[1],n[0]))));
    ssSetOutputPortWidth(S,1,(int_T)((MIN(n[1],n[0])*MIN(n[1],n[0]))));
    ssSetOutputPortWidth(S,2,(int_T)(MIN(n[1],n[0])*n[1]));
...
{
int_T    i, j, *n  = ssGetIWork(S);
real_T   *yu  = ssGetOutputPortRealSignal(S,0);
real_T   *ys  = ssGetOutputPortRealSignal(S,1);
real_T   *yv  = ssGetOutputPortRealSignal(S,2);

real_T   *rwv = ssGetRWork(S);
...
{	
	...
	for(j = 0; j < n[1]*n[0]; j++) yu[j] = rwv[j];
	for(j = 0; j < n[1]     ; j++) ys[(n[1]+1)*j] = rwv[n[1]*n[0]+j];
	for(j = 0; j < n[1]*n[1]; j++) yv[j] = rwv[n[1]*(n[0]+1)+j];
...
else
{
	...
	for(j = 0; j < n[1]*n[0]; j++) yv[j] = rwv[j];
	for(j = 0; j < n[0]     ; j++) ys[(n[0]+1)*j] = rwv[n[1]*n[0]+j];
	for(j = 0; j < n[0]*n[0]; j++) yu[j] = rwv[n[0]*(n[1]+1)+j];
